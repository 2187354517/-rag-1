...
const submitApi = async () => {
  ...
  // 3. 确保 user_name 存在
  if (!doc.user_name) {
    formData.append('user_name', user_id); // 默认值或从全局状态获取
  }
  // 遍历表单数据填充到formData
...
<template>
  <BaseDialog :title="title" v-model="visible" @closed="onClosed()">
    <el-form ref="docFormRef" :model="docForm" :rules="rules" label-width="120px" status-icon>
      ...
      <el-form-item label="上传文件" prop="file_name">
        <el-upload ref="uploadRef" action="#" drag accept=".pdf, .txt, .doc, .docx, .md" :limit="1"
          v-model:file-list="fileList" :auto-upload="false" :on-exceed="uploadOnExceed" :on-change="uploadOnChange"
          :on-remove="uploadOnRemove" class="upload-card">
  ...
  <el-dialog v-model="loading" title="上传进度" width="500" append-to-body :show-close="false"
