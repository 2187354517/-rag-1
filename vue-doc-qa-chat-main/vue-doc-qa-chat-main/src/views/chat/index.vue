...
const handleImageUpload = (file: File) => {
  ...
  reader.onload = (e) => {
    // 这里可以处理图片，比如显示预览或直接发送
    const imageData = e.target?.result as string
    // 你可以选择将图片直接发送或先显示预览
    humanInput.value = `[图片]${file.name}`
    // 实际发送时需要将imageData作为附件发送
  }
...
const onSubmit = async () => {
  ...
  const user_id = authStore.userInfo?.id // 获取当前用户ID
  ...
  if (!chatSessionId.value) {
    ...
    try {
      const res = await chatSessionsAddApi({
        title: humanInput.value,
        user_name: authStore.userInfo?.id  // 添加 user_id
      })
      ...
      <div class="chat-content">
        <div class="chat">
          ...
          <template v-for="(item, index) in chatting">
            ...
            <AssistantChat :key="index" v-if="item.role === 'assistant'" :content="item.content" :think="item.think"
              :is-stream="item.isStream" :hasThinkCard="true" :error="item.error"></AssistantChat>
          </template>
        </div>
        ...
        <div class="chat-input-container">
          ...
          <div class="input-card">
            <el-input v-model="humanInput" class="chat-send-input" type="textarea" :autofocus="true"
              :autosize="{ minRows: 3, maxRows: 20 }" :readonly="disabled" maxlength="2000" show-word-limit
              @keydown.enter="inputKeyboard" placeholder="请输入对话内容……">
            </el-input>

            <div class="bottom-send">
              <el-upload action="" :show-file-list="false" :before-upload="beforeImageUpload"
                :http-request="({ file }) => handleImageUpload(file)" accept="image/*">
                <el-button type="primary" plain round>
                  <UploadFilled size="20px" />
                </el-button>
              </el-upload>
            ...
            <div class="chat-send-bottom-controls">
              ...
              <div class="send-controls-extra">
                <el-button-group>
                  <el-popover :popper-style="{ borderRadius: '26px' }" :visible="historyVisible" placement="top-end"
                    :width="350" trigger="click" :persistent="false">
                    <template #reference>
                      <el-button round @click="historyVisible = !historyVisible">历史对话</el-button>
                    </template>
                    <ChatHistory v-model:visible="historyVisible" v-model:active-id="chatSessionId"
                      @item-click="onHistoryItemClick" @rename="onHistoryItemRename" @delete="onHistoryItemDelete" />
                  </el-popover>
