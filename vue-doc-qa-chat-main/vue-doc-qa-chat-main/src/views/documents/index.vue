...
const user_id = authStore.userInfo?.id
...
async function getTableList(params?: DocParamsType) {
  ...
  const res = await docPageApi(params || { page_num: 1, page_size: 10, user_name: user_id }).finally(() => (loading.value = false))
 ...
 * 文档向量化操作
 */
async function vectorAll(user_id) {
  loading.value = true
  tableLoadText.value = '向量化中……'

  try {
    await docVectorAllApi(user_id, { timeout: 600000 })
...
<template>
  <div class="main-full upload-box">
    ...
    <div class="card table-card">
      ...
      <el-table :data="tableDataList" v-loading="loading" :element-loading-text="tableLoadText" :border="true"
        class="table-main">
        ...
        <el-table-column align="center" label="操作" width="240">
          <template #default="scope">
            <el-button type="primary" link>
              <el-link type="primary" :underline="false" :icon="View" :href="'/api/documents/read/' + scope.row.id"
                target="_blank">
                查看
              </el-link>
            </el-button>
            <el-button type="primary" link :icon="EditPen" @click="onEdit(scope.$index, scope.row)">编辑</el-button>
            <el-popconfirm title="确定要删除该行吗？" width="180" placement="top-end"
      ...
      <el-pagination v-model:current-page="tableState.pageNum" v-model:page-size="tableState.pageSize"
        :page-sizes="[10, 20, 30, 40, 50]" :background="true" layout="total, sizes, prev, pager, next, jumper"
        :total="tableState.total" class="pagination-right" />
    </div>

    <write-form v-if="dialog.writeFormVisible" :open="dialog.writeFormVisible" :current-row="currentRow" @ok="onOk"
