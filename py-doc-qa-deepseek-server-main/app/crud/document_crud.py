...
class DocumentCrud:
    __BASE_PATH = "D:\\math-ai\\Project1\\"
    
    def __get_user_path(self, user_name: str):
        """获取用户专属文件存储路径"""
        return f"fileStorage1\\{user_name}"

    async def __save_file(self, file: UploadFile = File(), user_name: str = None):
        """I/O操作，写入文件"""
        ...
        save_path = Path(self.__BASE_PATH) / user_path / new_file_name
        new_path = Path(user_path) / new_file_name
    ...
    async def add(self, data: UploadFormData):
        """新增文档记录"""
        file = data.file
        new_file_name, new_file_path, new_suffix = await self.__save_file(file,data.user_name)
        doc = DocumentUpdate(
            name=data.name,
    ...
    async def update(self, data: UpdateFormData):
        ...
        with Session(engine) as session:
            ...
            if db_document.user_name != data.user_name:
    ...
    def page(self, params: DocumentParams):
        ...
        with Session(engine) as session:
            ...
            if hasattr(params, 'user_name'):
                count_query = count_query.where(Document.user_name == params.user_name)
                query = query.where(Document.user_name == params.user_name)
            # 根据 name 模糊查询，并查询总数
            if name:
