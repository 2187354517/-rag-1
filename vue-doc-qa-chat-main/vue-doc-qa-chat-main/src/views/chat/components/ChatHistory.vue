 ...
 * 加载会话列表
 */
async function loadData(user_name: string = user_id) {
  loading.value = true
  const res = await chatSessionsApi(user_name).finally(() => (loading.value = false))
            ...
            <template v-for="item in sessionList" :key="item.id">
              <div class="history-item" :class="{ active: activeId === item.id }">
                <div class="item-content">
                  <el-tooltip class="box-item" :disabled="!item.title || item.title.length < 15" :content="item.title"
                    placement="top-start">
                  ...
                  <el-dropdown placement="bottom-start" trigger="click" @command="onCommand">
                    <el-button class="extra" :class="{ active: activeId === item.id }" size="small" type="warning" plain
                      round>
                      <el-icon>
                        <More />
                      </el-icon>
